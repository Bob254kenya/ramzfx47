...
function addTick(symbol: string, digit: number, price: number) {
  ...
  if (globalTickHistory[symbol].length > MAX_HISTORY) globalTickHistory[symbol].shift();
  if (globalTickPrices[symbol].length > MAX_HISTORY) globalTickPrices[symbol].shift();
...
function calculateDigitStats(symbol: string, tickRange: number): DigitStats {
  ...
  for (let i = 0; i <= 9; i++) {
    percentages[i] = (frequency[i] / (recentTicks.length || 1)) * 100;
  }
...
function buildCandles(prices: number[], times: number[], tf: string): Candle[] {
  ...
  for (let i = 0; i < prices.length; i++) {
    ...
    const time = times[i] || Date.now() / 1000 + i;
...
function calcBollingerBands(prices: number[], period: number = 20, multiplier: number = 2) {
  ...
  const variance = recentPrices.reduce((sum, price) => sum + Math.pow(price - lastMiddle, 2), 0) / period;
...
const TPSLNotificationPopup = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
      <div className={`pointer-events-auto w-[500px] h-[300px] rounded-xl shadow-2xl overflow-hidden transition-all duration-300 ${
        isExiting ? 'opacity-0 translate-y-8' : 'opacity-100 translate-y-0'
      }`}>
        <div className={`relative w-full h-full overflow-hidden ${
          isTP ? 'bg-gradient-to-br from-emerald-500 to-emerald-700' : 'bg-gradient-to-br from-rose-500 to-rose-700'
        }`}>
          <div className="absolute inset-0 flex items-center justify-center flex-col p-6 text-center">
            <div className="text-6xl mb-4">{isTP ? '🎉' : '😢'}</div>
            <h3 className="text-2xl font-bold text-white mb-2">
              {isTP ? 'TAKE PROFIT!' : 'STOP LOSS!'}
            </h3>
            <p className="text-white/90 text-sm mb-3">{notification.message}</p>
            {amount && (
              <p className={`text-2xl font-bold mb-4 ${isTP ? 'text-emerald-200' : 'text-rose-200'}`}>
            ...
            <button
              ...
              className="px-6 py-2 bg-white/90 rounded-lg font-semibold hover:bg-white transition-all"
            >
              OK
            </button>
          </div>
...
export default function TradingChart() {
  ...
  const [botStats, setBotStats] = useState({ 
    trades: 0, wins: 0, losses: 0, pnl: 0, currentStake: 0, consecutiveLosses: 0 
  });
  ...
  const { frequency, percentages, mostCommon, leastCommon, totalTicks, 
    evenPercentage, oddPercentage, overPercentage, underPercentage } = digitStats;
  
  const riseSignal = useMemo(() => {
    const confidence = rsi < 30 ? 85 : rsi > 70 ? 25 : 50 + (50 - rsi);
    return { 
      direction: rsi < 45 ? 'Rise' : 'Fall', 
      confidence: Math.min(95, Math.max(10, Math.round(confidence))) 
    };
  }, [rsi]);

  const eoSignal = useMemo(() => {
    const confidence = Math.abs(evenPercentage - 50) * 2 + 50;
    return { 
      direction: evenPercentage > 50 ? 'Even' : 'Odd', 
      confidence: Math.min(90, Math.round(confidence)) 
    };
  }, [evenPercentage]);

  const ouSignal = useMemo(() => {
    const confidence = Math.abs(overPercentage - 50) * 2 + 50;
    return { 
      direction: overPercentage > 50 ? 'Over' : 'Under', 
      confidence: Math.min(90, Math.round(confidence)) 
    };
  }, [overPercentage]);
  ...
  const matchSignal = useMemo(() => {
    ...
    return { 
      digit: mostCommon, 
      confidence: Math.min(90, Math.round(bestPct * 3)) 
    };
  ...
  const totalTradesCount = tradeHistory.filter(t => t.status !== 'open' && !t.isVirtual).length;
  ...
  const parseCombinedPatterns = useCallback((input: string): string[] => {
    ...
    return input.split(',').map(p => p.trim().toUpperCase()).filter(p => p.length > 0);
  }, []);
  ...
  const checkStrategyCondition = useCallback((): boolean => {
    ...
    if (strategyMode === 'digit') return checkDigitCondition();
    ...
    try {
      ...
      if (newPnl >= tpValue) {
        if (typeof window !== 'undefined' && (window as any).showTPNotification) {
          (window as any).showTPNotification('tp', `Take Profit Target Hit!`, newPnl);
        }
        shouldStop = true;
      ...
      if (newPnl <= -slValue) {
        if (typeof window !== 'undefined' && (window as any).showTPNotification) {
          (window as any).showTPNotification('sl', `Stop Loss Target Hit!`, Math.abs(newPnl));
        }
        shouldStop = true;
  ...
  const simulateVirtualContract = useCallback((
  ...
  const startBot = useCallback(async () => {
    ...
    while (botRunningRef.current && !shouldStopRef.current) {
      ...
      if (strategyEnabled) {
        while (botRunningRef.current && !checkStrategyCondition() && !shouldStopRef.current) {
          await new Promise(r => setTimeout(r, 500));
        }
    ...
    if (voiceEnabled && (pnl <= -sl || pnl >= tp)) {
      ...
      hookEnabled, virtualLossCount, realCount, executeRealTrade, executeVirtualTrade, turboMode, refreshBalance]);
  ...
  // Update pattern list when input changes
  useEffect(() => {
  ...
  // Update digit stats periodically
  useEffect(() => {
    updateDigitStats();
  ...
  }, [selectedContractType, selectedPrediction, updateDisplaySymbols, digitStats.last26Digits, digitStats.tickPrices]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // Clear and set global tick history
  ...
  // Simplified chart rendering (placeholder for the full rendering logic)
  useEffect(() => {
    if (!showChart || !canvasRef.current || candles.length < 2) return;
    ...
    const candleEndIndices = mapCandlesToPriceIndices(prices, times, timeframe);
    const gap = 1;
    const totalCandleW = candleWidth + gap;
    const chartW = rect.width - 70;
    const offsetX = 5;
    
    const visibleCandles = candles.slice(-Math.floor(chartW / totalCandleW));
    ...
    visibleCandles.forEach((candle, i) => {
      ...
      // Simplified drawing
      ctx.fillStyle = color;
      ctx.fillRect(x, 100, candleWidth, 50);
    });
  }, [candles, prices, times, timeframe, candleWidth, scrollOffset, showChart]);
        ...
        <div className="flex items-center gap-2">
          <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-[10px] ${
            ...
            {derivApi.isConnected ? (
              ...
              <><WifiOff className="w-3 h-3" /><span>Disconnected</span></>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-1">
          {TIMEFRAMES.map(tf => (
            <Button key={tf} size="sm" variant={timeframe === tf ? 'default' : 'outline'}
              className="h-7 text-xs px-3" onClick={() => setTimeframe(tf)}>
              {tf}
            </Button>
          ))}
        </div>
        ...
        <div className="xl:col-span-8 space-y-3">
          ...
          <div className="bg-card border border-border rounded-xl p-3 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" className="h-6 text-[10px] gap-1" onClick={() => setShowIndicatorPanel(!showIndicatorPanel)}>
            ...
            {showIndicatorPanel && (
              <div className="flex flex-wrap gap-2 p-2 bg-muted/30 rounded-lg">
                {(['RSI', 'BB', 'MA', 'MACD'] as const).map(type => (
                  <Button key={type} size="sm" variant="outline" className="h-6 text-[9px]"
                    onClick={() => addIndicator(type)} disabled={indicators.some(i => i.type === type)}>
                    + {type}
                  </Button>
                ))}
              </div>
            ...
            {indicators.length > 0 ? (
              <div className="space-y-1.5">
                {indicators.map(indicator => (
                  <div key={indicator.id} className="flex items-center justify-between p-1.5 bg-muted/20 rounded-lg">
                    <div className="flex items-center gap-2">
                      <Switch checked={indicator.enabled} onCheckedChange={() => toggleIndicator(indicator.id)} className="scale-75" />
                      <span className="text-[10px] font-mono">{indicator.type}</span>
                    </div>
                    <Button size="sm" variant="ghost" className="h-5 w-5 p-0 text-muted-foreground hover:text-loss"
              ...
              <div className="text-center py-4 text-muted-foreground text-xs">
                No indicators added. Click "Add Indicator" to add technical indicators.
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 md:grid-cols-7 gap-2">
            {[
              ...
              { label: 'RSI', value: rsi.toFixed(1), color: 'text-[#D29922]' },
          ...
          <div className="bg-card border border-border rounded-xl p-3 space-y-3">
            ...
            <div className="grid grid-cols-5 md:grid-cols-10 gap-1.5">
              {Array.from({ length: 10 }, (_, d) => {
                ...
                return (
                  <button key={d} onClick={() => setSelectedPrediction(String(d))}
                  ...
                  >
                    ...
                    {isBestDiffer && <Badge className="absolute -top-1 -left-1 text-[7px] px-1 bg-loss text-loss-foreground">Diff</Badge>}
                  </button>
                );
              })}
            </div>
        ...
        <div className="xl:col-span-4 space-y-3">
          ...
          <div className="bg-card border border-primary/30 rounded-xl p-3">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant={voiceEnabled ? 'default' : 'outline'} className="h-7 text-[10px] gap-1" onClick={() => {
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button size="sm" variant={turboMode ? 'default' : 'outline'}
                  className={`h-6 text-[9px] px-2 ${turboMode ? 'bg-profit hover:bg-profit/90 text-profit-foreground' : ''}`}
                  onClick={() => setTurboMode(!turboMode)} disabled={botRunning}>
              ...
              {hookEnabled && (
                <div className="space-y-2 p-2 bg-primary/5 rounded-lg border border-primary/20">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <Input type="number" min="1" max="20" value={virtualLossCount}
                        onChange={e => setVirtualLossCount(e.target.value)} disabled={botRunning} className="h-7 text-[10px]" />
                    </div>
                    <div>
                      <label className="text-[8px] text-muted-foreground">Real Trades (max)</label>
                      <Input type="number" min="1" max="10" value={realCount}
                        onChange={e => setRealCount(e.target.value)} disabled={botRunning} className="h-7 text-[10px]" />
                    </div>
                  </div>
                  ...
                  {vhStatus === 'confirmed' && botRunning && (
                    <div className="bg-profit/10 border border-profit/30 rounded p-1 text-[8px] text-profit text-center">
                ...
                <div className="space-y-2">
                  <div className="flex gap-1">
                    ...
                    <Button size="sm" variant={strategyMode === 'combined' ? 'default' : 'outline'}
                      className="text-[9px] h-6 px-2 flex-1" onClick={() => setStrategyMode('combined')} disabled={botRunning}>
                  ...
                  {strategyMode === 'pattern' ? (
                    ...
                    <div>
                      ...
                      <Textarea placeholder="e.g., 112, 55, OEE, OVER, 99U" value={combinedPatternsInput}
                        onChange={e => setCombinedPatternsInput(e.target.value.toUpperCase())}
                        disabled={botRunning} className="h-20 text-[10px] font-mono min-h-0 mt-1" />
                      ...
                      {combinedPatternsList.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-1">
                          {combinedPatternsList.map((p, idx) => (
                            <Badge key={idx} variant="outline" className="text-[8px] font-mono">{p}</Badge>
                          ))}
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-card border border-border rounded-xl p-3">
            <h3 className="text-xs font-semibold text-foreground mb-2 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-[8px] animate-pulse">LIVE</Badge>
            ...
            <div className="flex flex-wrap gap-3 mb-3 text-[8px] justify-center">
              ...
              {legend.symbol3 && (
                <div className="flex items-center gap-1">
                  <div className={`w-4 h-4 rounded flex items-center justify-center text-[9px] font-bold ${
                    legend.symbol3 === 'C' ? 'bg-muted/20 text-foreground' :
                    'bg-muted/20 text-foreground'
                  }`}>
                    {legend.symbol3}
                  </div>
          ...
          <div className="bg-card border border-border rounded-xl p-3 space-y-2">
            ...
            <div className="grid grid-cols-4 gap-1.5">
              <div className="bg-muted/30 rounded-lg p-1.5 text-center">
                ...
                <div className="font-mono text-sm font-bold text-foreground">{totalTradesCount}</div>
              </div>
